...
public final class Chatter {
  ...
  static {
    ...
    APPLICATIONS.add(
        new Application(
            "DS Emulators (DeSmuME, Citra, etc.)",
  ...
  /** Sets up random mode. This mode randomly generates inputs for the application. */
  ...
  private static void setupRandomMode() {
    while (true) {
      ...
      if (windowTitle == null
          || application.getWindowTitles().stream().noneMatch(windowTitle::contains)) {
  ...
  /**
   * Sets up Twitch monitoring. This mode listens for inputs from the broadcaster's Twitch chat.
   *
  ...
  private static void setupTwitchMonitoring(@NotNull Scanner scanner) {
    ...
    if (channel.length() > 25 || channel.length() < 4 || channel.matches(".*\\W.*")) {
      log.warn(
          "Invalid channel username. It must be between 4 and 25 characters"
            ...
            event
                ...
                .ifPresent(
                    message -> {
                      ...
                      String windowTitle = getForegroundWindowTitle();
                      if (windowTitle == null
                          || application.getWindowTitles().stream()
                              .noneMatch(windowTitle::contains)) {
                        log.info(
                            "Ignoring message: {} (application not in focus) | {}", message, event);
                        return;
                      }
                                  ...
                                  () -> {
                                    ...
                                    if (!inputQueue.isEmpty()) {
                                      ...
                                      if (mostFrequentInputName != null) {
                                        // Find the corresponding InputRequest for the most frequent
                                        // input
                                        for (InputRequest request : inputQueue) {
                                          if (request
                                              .getInput()
                                              .getInputName()
                                              .equalsIgnoreCase(mostFrequentInputName)) {
                                            boolean isLongInput =
                                                request
                                                    .getMessage()
                                                    .toUpperCase(Locale.ROOT)
                                                    .equals(request.getMessage());
                                            log.info("Processing input: {}", request.getMessage());

                                            if (request.getInput() instanceof KeyInput) {
                                              startKeyInput(
                                                  (KeyInput) request.getInput(), isLongInput);
                                            } else if (request.getInput() instanceof MouseInput) {
                                              startMouseInput(
                                                  (MouseInput) request.getInput(), isLongInput);
                                            }
                                            break;
                                          }
                                        }
    ...
    for (InputRequest request : inputQueue) {
      String inputName =
          request
              .getInput()
              .getInputName()
              .toLowerCase(Locale.ROOT); // Normalize input name to lower case
  ...
  private static void startKeyInput(@NotNull KeyInput keyInput, boolean longInput) {
    ...
    boolean mouseInput =
        keyCode == InputEvent.BUTTON1_DOWN_MASK || keyCode == InputEvent.BUTTON2_DOWN_MASK;
        ...
        new Thread(
            () -> {
              try {
                ...
                long duration =
                    longInput ? keyInput.getLongDuration() : keyInput.getShortDuration();
        ...
        new Thread(
            () -> {
              try {
                ...
                if (MyUser32.INSTANCE.getWindowRect(
                    MyUser32.INSTANCE.getForegroundWindow(), rect)) {
                  ...
                  long duration =
                      longInput ? mouseInput.getLongDuration() : mouseInput.getShortDuration();
  ...
  /**
   * Interface for the user32.dll library. This is used to get the title of the current foreground
   * window.
   */
